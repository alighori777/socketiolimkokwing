...
import { getApplicantBalanceBreakdown, getApplicantTransHistory } from '../../../ducks/actions';
...
export default (props) => {
  ...
  const { id } = useParams();
  const outstandingData = useSelector((state) => state.applicationForm.balanceBreakdown);
  const transactionData = useSelector((state) => state.applicationForm.balanceHistory);
  ...
  useEffect(() => {
    dispatch(getApplicantBalanceBreakdown(id));
    dispatch(getApplicantTransHistory(id));
  }, []);
    ...
    return {
      onClick: () => {
        history.push({
          ...
          state: { aid: id, ref: record?.invoice_no, balance: record?.amount },
        });
      },
    };
  };
      ...
      <Col span={24}>
        <Space direction="vertical" size={20} className="w-100">
          ...
          <Table
            ...
            bordered={false}
            columns={ListCol1}
            dataSource={transactionData?.rows}
            pagination={false}
          />
        </Space>
      </Col>
