...
export default (props) => {
    ...
    useEffect(() => {
        if (data && data.qualifications && data.qualifications.length) {
            ...
            setValue('certificate', {fileList: [{uid: '-1', name: getFileName(qdata.certificate), status: 'done', url: `${baseUrl}${qdata.certificate}`}]});
           
        }
        if (data && data.documents && data.documents.length) {
            let CV =  data.documents.find(x => x.document_name == 'CV')
            let port =  data.documents.find(x => x.document_name == 'Portfolio')
            if(CV) {
                setValue('resume_cv', {fileList: [{uid: '-1', name: getFileName(CV.document), status: 'done', url: `${baseUrl}${CV.document}`}]});
            }

            if(port) {
                setValue('portfolio', {fileList: [{uid: '-1', name: getFileName(port.document), status: 'done', url: `${baseUrl}${port.document}`}]});
            }
        }
        if (data && data?.students_programs && data?.students_programs.length) {
            let qdata = selectProg ? data?.students_programs.find(x => x.program.program == selectProg) : data?.students_programs.find(x => x.program.program_status == 'Active');
            setValue('first_pref', {label: qdata.first_pref_name, value: qdata.first_pref});
            setValue('second_pref', {label: qdata.second_pref_name, value: qdata.second_pref});
            setValue('third_pref', {label: qdata.third_pref_name, value: qdata.third_pref});
        }
    }, [data, selectProg]);
